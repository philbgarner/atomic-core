...
export function makeDungeon() {
	...
	renderer = createDungeonRenderer(globs.viewportEl, window.game, {
		...
		onCellHover(info) { handleCellHover(info); },

		// ── Click: yellow region highlight + texture mapping panel ───────────
		onCellClick(info) { handleCellClick(info); },
	});
	...
	renderer.addLayer({
		...
		filter: (cx, cz, direction) =>
			isOdd(cx, cz, direction)
				? { tile: "natural/wall_forest2" }
				: null,
	});
...
export async function buildSkyboxFaces() {
	const side = () => makeFaceUrl(512, 512, [[0, ZENITH], [0.5, HORIZON], [1, '#000000']], 999);
	const [px, nx, pz, nz] = await Promise.all([side(), side(), side(), side()]);

	// Top face: starfield.
	const py = await makeFaceUrl(512, 512, [[0, ZENITH], [1, ZENITH]], 999);
