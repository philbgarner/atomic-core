...
export const Connectscreen = (() => {
  ...
  function setup() {
    ...
    globs.connectBtn.addEventListener("click", async () => {
      ...
      const chosenSprite =
        document.querySelector('input[name="sprite"]:checked')?.value ??
        "terenia";
      ...
      try {
        ...
        globs.connectError.textContent =
  ...
  async function startGame(
    ...
    console.trace(
      "initPartyData(playerId, chosenSprite)",
      initPartyData(playerId, chosenSprite),
    );
    ...
    addLog(
      `Dungeon 0x${seed} — ${roomCount} rooms, ${corridorCount} corridors`,
      "turn",
    );
