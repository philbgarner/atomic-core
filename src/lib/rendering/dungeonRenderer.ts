...
export function createDungeonRenderer(
  ...
  const skyPanelLightColor = new THREE.Color(
    ...(outsideLight.color ?? ([0, 0, 0] as [number, number, number])),
  );
  ...
  if (packedAtlas) {
    sharedAtlasTex = new THREE.Texture(
      packedAtlas.texture as HTMLCanvasElement,
    );
  ...
  if (packedAtlas) {
    ...
    tileUvLookupTex = new THREE.DataTexture(
      uvData,
      tileUvCount,
      1,
      THREE.RGBAFormat,
  ...
  function setSkirtLookupUniform(
    mat: THREE.Material,
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      uniforms: THREE.UniformsUtils.merge([
        ...
        makeBasicAtlasUniforms({
          ...
          fogFar,
          ...(tileUvLookupTex
            ? { tileUvLookup: tileUvLookupTex, tileUvCount }
            : {}),
  ...
  function buildLayerMesh(spec: LayerSpec): THREE.InstancedMesh | null {
    ...
    function tryAdd(
      ...
      const id =
        result.tile !== undefined ? resolveTile(result.tile, resolver) : 0;
      ...
      for (let cx = 0; cx < width; cx++) {
        ...
        if (spec.target === "ceil" && ceilVal !== 0) {
          tryAdd(
            ...
            makeFaceMatrix(
              wx,
              -(ceilVal - 128) * offsetStep + ceilingH,
              wz,
              HALF_PI,
              0,
              0,
        ...
        if (spec.target === "floorSkirt" && floorVal !== 0) {
          ...
          if (nfN !== null && nfN < floorVal)
            tryAddFloorSkirtTiled(nfN, wx, cz * tileSize, Math.PI, "north");
          const nfS = openFloorVal(cx, cz + 1);
          if (nfS !== null && nfS < floorVal)
            tryAddFloorSkirtTiled(nfS, wx, (cz + 1) * tileSize, 0, "south");
          const nfW = openFloorVal(cx - 1, cz);
          if (nfW !== null && nfW < floorVal)
            tryAddFloorSkirtTiled(nfW, cx * tileSize, wz, -HALF_PI, "west");
          const nfE = openFloorVal(cx + 1, cz);
          if (nfE !== null && nfE < floorVal)
            tryAddFloorSkirtTiled(
              nfE,
              (cx + 1) * tileSize,
              wz,
              HALF_PI,
    ...
    const mesh = buildInstancedMesh(
      ...
      rotations,
      spec.target === "ceilSkirt" || spec.target === "floorSkirt"
        ? heightScales
        : undefined,
    ...
    if (sm.uniforms?.["uSurfaceLight"]) {
      const isWallLike =
        spec.target === "wall" ||
        spec.target === "floorSkirt" ||
        spec.target === "ceilSkirt";
      if (isWallLike) {
  ...
  function buildDungeon() {
    ...
    if (ceilHeightTex) {
      for (const mat of [floorMat, floorEdgeMat, floorWallSkirtMat]) {
        if (mat instanceof THREE.ShaderMaterial)
          (
            mat.uniforms["uCeilHeightLookup"] as { value: THREE.Texture }
          ).value = ceilHeightTex;
      }
    }
    ...
    const ceilOffData = outputs.textures.ceilingHeightOffset?.image.data as
      | Uint8Array
      | undefined;
    const skyPanelCountData = outputs.textures.skyPanelCount?.image.data as
      | Uint8Array
      | undefined;
    const ceilingPanelCountData = outputs.textures.ceilingPanelCount?.image
    ...
    const openSkyLighting = Math.max(
      0,
      Math.min(1, options.openSkyLighting ?? 0),
    );
    ...
    for (let cz = 0; cz < height; cz++) {
      for (let cx = 0; cx < width; cx++) {
        ...
        if (floorVal !== 0) {
          ...
          if (aoEnabled) {
            ...
            if (openSkyLighting > 0) {
              const adj =
                !isOpenSky &&
                (isOpenSkyCeil(cx, cz - 1) ||
                  isOpenSkyCeil(cx, cz + 1) ||
                  isOpenSkyCeil(cx - 1, cz) ||
                  isOpenSkyCeil(cx + 1, cz));
              const boost = isOpenSky
                ? openSkyLighting
                : adj
                  ? openSkyLighting * 0.5
                  : 0;
        ...
        if (isSolid(cx, cz - 1)) {
          ...
          const nfloorVal = floorOffData
            ? (floorOffData[idx - width] ?? 128)
            : 128;
        ...
        if (floorVal !== 0) {
          ...
          const nfN = openFloorVal(cx, cz - 1);
          if (nfN !== null && nfN < floorVal)
            addFloorSkirt(nfN, wx, cz * tileSize, Math.PI, "north");
          const nfS = openFloorVal(cx, cz + 1);
          if (nfS !== null && nfS < floorVal)
            addFloorSkirt(nfS, wx, (cz + 1) * tileSize, 0, "south");
          const nfW = openFloorVal(cx - 1, cz);
          if (nfW !== null && nfW < floorVal)
            addFloorSkirt(nfW, cx * tileSize, wz, -HALF_PI, "west");
          const nfE = openFloorVal(cx + 1, cz);
          if (nfE !== null && nfE < floorVal)
            addFloorSkirt(nfE, (cx + 1) * tileSize, wz, HALF_PI, "east");
        }
        ...
        if (floorVal !== 0) {
          ...
          if (isSolid(cx, cz - 1)) {
            ...
            const nfloorVal = floorOffData
              ? (floorOffData[idx - width] ?? 128)
              : 128;
            const nceilVal = ceilOffData
              ? (ceilOffData[idx - width] ?? 128)
              : 128;
            const nfvo = Math.min((nfloorVal - 128) * offsetStep, fvo);
            const ncvo = isOpenSky
              ? nceilVal
                ? -(nceilVal - 128) * offsetStep
                : nfvo - tileSize
              : cvo;
        ...
        if (skyCount > 0) {
          ...
          if (isOpenSky) {
            ...
            // handles interior to skylight
            if (isOpenSkyCeil(cx, cz - 1))
              emitSkyPanels(wx, cz * tileSize, 0, cvo);
            if (isOpenSkyCeil(cx, cz + 1))
              emitSkyPanels(wx, (cz + 1) * tileSize, Math.PI, cvo);
            if (isOpenSkyCeil(cx - 1, cz))
              emitSkyPanels(cx * tileSize, wz, +HALF_PI, cvo);
            if (isOpenSkyCeil(cx + 1, cz))
              emitSkyPanels((cx + 1) * tileSize, wz, -HALF_PI, cvo);
          }
        }
        ...
        const ceilPanelCount = ceilingPanelCountData
          ? (ceilingPanelCountData[idx] ?? 0)
          : 0;
        if (ceilPanelCount > 0) {
          function emitCeilPanels(mx: number, mz: number, ry: number) {
            for (let i = 0; i < ceilPanelCount; i++) {
              ceilPanelEdges.push(
                makeFaceMatrix(
                  mx,
                  ceilingH - i * tileSize - tileSize / 2,
                  mz,
                  0,
                  ry,
                  0,
          ...
          if (isSolid(cx, cz + 1))
            emitCeilPanels(wx, (cz + 1) * tileSize, Math.PI);
          if (isSolid(cx - 1, cz)) emitCeilPanels(cx * tileSize, wz, HALF_PI);
          if (isSolid(cx + 1, cz))
            emitCeilPanels((cx + 1) * tileSize, wz, -HALF_PI);
        }
      }
    }
    ...
    floorMesh = buildInstancedMesh(
      ...
      floorMat,
      !!packedAtlas,
      undefined /*new Float32Array(floorOffsets)*/,
      undefined,
      undefined,
      floorCX,
      floorCZ,
    ...
    ceilMesh = buildInstancedMesh(
      ...
      ceilMat,
      !!packedAtlas,
      undefined /*new Float32Array(ceilOffsets)*/,
      undefined,
      undefined,
      ceilCX,
      ceilCZ,
    ...
    floorEdgeMesh = buildInstancedMesh(
      ...
      fEdgeCX,
      fEdgeCZ,
      aoEnabled && floorEdgesAo.length
        ? new Float32Array(floorEdgesAo)
        : undefined,
    );
    ...
    ceilEdgeMesh = buildInstancedMesh(
      ...
      cEdgeCX,
      cEdgeCZ,
      aoEnabled && ceilEdgesAo.length
        ? new Float32Array(ceilEdgesAo)
        : undefined,
    );
    ...
    if (floorWallSkirtEdges.length > 0) {
      ...
      floorWallSkirtMesh = buildInstancedMesh(
        ...
        undefined,
        floorWallSkirtRots,
        floorWallSkirtHeightScales,
        fwsCX,
        fwsCZ,
        aoEnabled && floorWallSkirtAo.length
          ? new Float32Array(floorWallSkirtAo)
          : undefined,
        undefined,
        floorWallSkirtRowIndexes,
        floorWallSkirtUvOffsets,
      );
    ...
    if (ceilWallSkirtEdges.length > 0) {
      ...
      ceilWallSkirtMesh = buildInstancedMesh(
        ...
        undefined,
        ceilWallSkirtRots,
        ceilWallSkirtHeightScales,
        cwsCX,
        cwsCZ,
        aoEnabled && ceilWallSkirtAo.length
          ? new Float32Array(ceilWallSkirtAo)
          : undefined,
        undefined,
        ceilWallSkirtRowIndexes,
      );
  ...
  const onTurn = () => {
    ...
    tgtY =
      tileSize * eyeHeightFactor + getFloorOffset(game.player.x, game.player.z);
  ...
  function getCellAtPointer(clientX: number, clientY: number): CellInfo | null {
    ...
    if (hit.object.userData.entity) // is an entity
    {
      ...
      const { width } = outputs;
      const regionData = outputs.textures.regionId?.image.data as
        | Uint8Array
        | undefined;
  ...
  return {
    ...
    editEntity(entity, layerIndex: number, tile: string) {
      if (billboardMap.has(entity.id))
        billboardMap.get(entity.id)?.setLayerTile(tileSize, layerIndex, tile);
      else if (objectBillboardMap.has(entity.id))
        objectBillboardMap
          .get(entity.id)
    ...
    highlightCells(filter) {
      ...
      const regionData = outputs?.textures.regionId?.image.data as
        | Uint8Array
        | undefined;
      const solid = outputs?.textures.solid?.image.data as
        | Uint8Array
        | undefined;
      const width = outputs?.width ?? 0;
      const height = outputs?.height ?? 0;
    ...
    setSurfaceLighting(opts) {
      if (opts.floor !== undefined && floorMat instanceof THREE.ShaderMaterial)
        (floorMat.uniforms["uSurfaceLight"] as { value: number }).value =
          opts.floor;
      if (opts.ceiling !== undefined && ceilMat instanceof THREE.ShaderMaterial)
        (ceilMat.uniforms["uSurfaceLight"] as { value: number }).value =
          opts.ceiling;
      if (opts.wallMin !== undefined || opts.wallMax !== undefined) {
        for (const mat of atlasMaterials) {
          if (opts.wallMin !== undefined)
            (mat.uniforms["uWallLightMin"] as { value: number }).value =
              opts.wallMin;
          if (opts.wallMax !== undefined)
            (mat.uniforms["uWallLightMax"] as { value: number }).value =
              opts.wallMax;
        }
      }
    },
    setOutsideLight(opts) {
      if (
        ...
        skyPanelMat instanceof THREE.ShaderMaterial
      )
        (skyPanelMat.uniforms["uSurfaceLight"] as { value: number }).value =
          opts.brightness;
      if (opts.color !== undefined) {
        ...
        for (const mat of [
          ...
          if (mat instanceof THREE.ShaderMaterial)
            (
              mat.uniforms["uSkyLightColor"] as { value: THREE.Color }
            ).value.setRGB(r, g, b);
        }
      }
    },
    setSnapCameraToFloor(enabled: boolean) {
      ...
      if (enabled && initialized) {
        ...
        if (floorOffData && outputs) {
          ...
          const floorVal = floorOffData[idx] ?? 128;
          const floorOffset =
            floorVal !== 0 ? (floorVal - 128) * offsetStep : 0;
