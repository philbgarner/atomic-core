// src/lib/passages/traversal.ts
//
// Passage traversal state machine.
// Models step-by-step movement through a hidden passage tunnel.
// Traversal is driven by the same step-loop that drives auto-walk.
...
export function consumePassageStep(
  ...
  const cell = state.remainingCells[0]!;
  const rest = state.remainingCells.slice(1);
