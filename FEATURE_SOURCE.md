...
Each cell tracks `floorElevation` (from `floorHeightOffset`, converted to the same abstract mass-equivalent units as `mass` — one elevation step ≈ one full cell of fluid) and `mass` — the real depth of fluid above that cell's own floor, 0 when dry and *unbounded* above `MAX_MASS` (a deep pit's lowest cell legitimately holds much more than "one full cell's worth" once its connected pool has enough volume to reach the rim). Flow between the 4-neighborhood is driven by `surfaceHeight = floorElevation + mass`, split across two independent fixed-tick passes per `stepFluid()` call:
- **Fast pass** (15 Hz): neighbors whose *floor* (not surface) is at least one elevation step lower get a transfer toward `fastPassTarget(totalMass, drop)` — the split where both cells would share one `surfaceHeight` given the actual structural `drop` between them — capped at one cell's mass per tick, which is what makes fluid rush into a deep depression instead of trickling in. The target must scale with the real `drop`, not a fixed constant: since a pair that ever qualifies as "steep" (drop ≥ `STEEP_DROP_THRESHOLD`) stays gated into this pass permanently, capping the target near `MAX_MASS` regardless of how deep the drop actually is would leave a multi-step pit's deepest cell stuck there forever, never reaching a shallower connected neighbor's true water level — a real bug in an earlier version of this pass, ported from the side-on original's fixed near-`MAX_MASS` resting cap (correct there, where "lower neighbor" always meant exactly one grid cell down; wrong here, where a pit can be arbitrarily deep). Gating on floor elevation alone (not the combined surfaceHeight) matters too: gating on surfaceHeight would also fire between two same-elevation cells whenever one is full and the other empty, incorrectly fast-falling sideways across flat ground.
...
Both passes (plus `evaporate()`) iterate `FluidField.openCells` — a flat list of non-solid cell indices computed once at construction — instead of scanning every `width*height` cell and checking `isSolid` each time; and both write into `FluidField.scratch`, a persistent double-buffer reused across calls (via `.set(mass)` each tick) rather than allocating a fresh `Float32Array` with `.slice()`. At up to 300 damped ticks/sec this avoided allocation is the bigger win — it was previously the dominant source of per-turn GC pressure/latency, independent of how much of the dungeon actually has fluid in it. `openCells`/`scratch` are engine-owned bookkeeping (documented as such on the type), not meant to be read or mutated by consumers, and stay valid only because nothing mutates `isSolid` after a field is constructed.
...
**Evaporation**: once per `stepFluid()` call (real `dt`, not the fixed-tick passes above — mirrors how the source ran its chemistry pass), any non-solid cell holding less than `EVAPORATE_THRESHOLD` mass drains at `EVAPORATE_RATE` mass/sec until it clears to 0. This is what makes fluid spread thin across open flat ground fade away over time instead of sitting there forever, while leaving real pools untouched: a pool's low point holds mass at or above `MAX_MASS` by construction of the fast/damped passes, well above the threshold, so no separate slope/neighbor detection is needed to tell "pooling" from "spread too thin to collect."

Rendering is a separate concern: one flat top quad per non-solid cell, world-positioned to exactly cover that cell's footprint, plus a vertical wall quad on every one of its 4 edges — allocated for every open cell but only actually rendered where an edge is a genuine boundary. Each cell's 4 top-quad corners are averaged with whichever of the (up to 3) cells sharing that grid point are non-solid and hold the same fluid type (`cornerHeight()` in `fluidMask.ts`) — this is what makes two connected same-type cells' shared corner land on one Y instead of stair-stepping, while a real dry/solid/different-fluid boundary (no compatible neighbor) still falls back to the cell's own flat height. A wall on a given edge only draws when that edge's neighbor is solid/off-grid or a *different* fluid type — a same-type open neighbor is already bridged seamlessly by the shared, averaged corner, so drawing a wall there too would just be a stray vertical seam visible through the transparent surface (e.g. the internal rings of a stepped pool). Different fluid types at the same floor elevation genuinely need that wall even on flat ground, since they never level with each other (the CA's compatible-neighbor check only flows same-type-or-empty). Where a wall does draw, its top edge is the same averaged corner used by the top quad (so it slopes with the surface instead of sitting flat), and each corner still takes the lower of its two contributing edges' bottoms (`MIN_WALL_HEIGHT` floor) so adjacent active walls meet without a crack. All geometry uses per-vertex height/depth/type attributes updated directly on the `BufferGeometry` (not sampled from a texture in the vertex shader — vertex texture fetch was tried first and produced broken geometry on a software/headless GL backend). World Y uses a *single* scale factor applied to the whole `surfaceHeight` sum (not separate scales for elevation vs. mass) — splitting the scale breaks the equal-`surfaceHeight`-implies-equal-worldY invariant the flat-surface guarantee depends on, making the rendered surface visibly non-flat or clip into the floor across any stepped pit. To make a single fresh pour look shallower, reduce the poured `amount` instead.

The per-vertex "depth" attribute driving opacity/visibility is real world-space water depth (`mass * worldUnitsPerStep`), not `mass / MAX_MASS`. `mass` already *is* physical depth above that cell's own floor (in the same mass-equivalent units as `floorElevation`) — a basin's shallow shoreline cells legitimately hold less mass than its deep center even once fully settled and level, so normalizing against the fixed `MAX_MASS` cap made shoreline opacity track the underlying floor's shape rather than the (already flat) water surface, visually reading as an unleveled tilt or a hard cutoff edge even though the geometry itself was flat. Two new uniforms (`uMinVisibleDepth`, `uOpacityDepth`, both scaled off `worldUnitsPerStep`) replace the old fixed 0.02/1.5 constants that assumed depth was already a 0-1 fraction.

The fragment shader also supports optional per-fluid-type `refractionIndex`, `opacity`, `glowColor`, and `glowIntensity` (all on `FluidDef`, all optional). Since this renderer has no scene-capture/render-to-texture pass, `refractionIndex` doesn't truly bend whatever's behind the surface — it drives a cheap, static, position-based brightness ripple as a stand-in for the look of wavy water. `glowColor`/`glowIntensity` add a flat additive tint on top (not light-reactive), useful for telegraphing a dangerous or magical fluid (e.g. lava) regardless of scene lighting. All four are also live-editable per type via `FluidSurfaceHandle.setFluidProperty(typeId, patch)`, which pokes the underlying uniform arrays directly with no geometry rebuild.

**Files:**
- `fluid/fluid.ts` — `FluidField` struct (`cellType`, `mass`, `floorElevation`, `isSolid` typed arrays + `version`/tick accumulators, plus engine-owned `openCells`/`scratch` performance bookkeeping — see above); `createFluidField()`, `fluidFieldFromDungeon(outputs)` (reads `solid`/`colliderFlags`/`floorHeightOffset` straight from `DungeonOutputs.textures`, treating the `floorHeightOffset` pit sentinel as solid-for-fluid); `stepFluid(field, dt)` fixed-timestep accumulator loop running the fast/damped passes plus the once-per-call `evaporate()` pass above; query `fluidCellAt()`/`fluidDepthAt()`; mutator `placeFluidCircle()`; generic `FluidDef { name, color, density, refractionIndex?, opacity?, glowColor?, glowIntensity? }` config type (consumer-supplied `Record<number, FluidDef>`, no hardcoded type table)
- `rendering/fluidMask.ts` — `FluidMask` + `createFluidMask()`/`updateFluidMask()`: general-purpose R8 `DataTexture` pair (depth, type) mirroring `temperatureMask.ts`'s pattern, independent of the mesh helper below, for custom visualizations (e.g. a minimap overlay); `createFluidSurface(renderer, field, options)` builds the top-quad-plus-side-wall mesh described above (with corner-averaged, cross-cell-smoothed top surfaces and the refraction/opacity/glow shader) and returns a `FluidSurfaceHandle` with `sync()` (call after `stepFluid` or any `placeFluidCircle` mutation), `setFluidProperty(typeId, patch)` (live per-type refraction/opacity/glow updates, no rebuild), and `remove()`
