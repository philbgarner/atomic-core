...
**Files:**
- `rendering/dungeonRenderer.ts` — main Three.js scene, render loop, shader uniforms; supports per-cell floor/ceiling height offsets via `aHeightOffset` and per-instance UV rotation via `aUvRotation`; per-direction tile specs via `wallTiles`, `floorSkirtTiles`, `ceilSkirtTiles` options; uses `basicLighting.ts` shaders
- `rendering/basicLighting.ts` — minimal atlas and object shaders: texture sampling + linear fog; `aUvRotation` attribute rotates UVs within tile bounds in 90° steps (0–3); no torch flicker or tint bands; used by `dungeonRenderer.ts`
