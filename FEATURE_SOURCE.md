...
- `rendering/basicLighting.ts` — minimal atlas and object shaders: texture sampling + linear fog; `aUvRotation` rotates UVs in 90° steps (0–3); `aUvHeightScale` clips UVs to the top fraction of a tile (top-aligned) so partial-height skirt panels keep brick aspect ratio; `aCell` per-instance vec2 attribute (packed from two floats to stay within the 16-slot WebGL limit) drives the overlay UV; 4-slot surface-painter overlay composited in fragment shader; `uSkirtLookup` uniform provides 4 additional overlay slots for skirt meshes (same RGBA encoding, defaults to 1×1 zero texture = no-op); `aAoCorners` per-instance vec4 attribute carries pre-baked AO corner values (tl/tr/bl/br, [0,1]) for floor, ceiling, and wall faces; vertex shader selects the correct corner value per vertex using raw UV and outputs `vAo` varying; fragment shader darkens by `mix(1 - uAoIntensity, 1, vAo)` before fog; `uAoIntensity=0` (default) disables the pass at zero cost; directional surface lighting: `aFaceN` per-instance vec2 attribute (packed) encodes the XZ outward normal for wall faces; `uSurfaceLight` uniform (`>= 0` = fixed multiplier for floor/ceiling, `< 0` = use camera-angle formula); `uCamDir` uniform (XZ camera forward, updated per frame); `uWallLightMin`/`uWallLightMax` uniforms drive the wall formula `uWallLightMin + abs(dot(normal, camFwd)) * (uWallLightMax - uWallLightMin)`; all four values configurable via `surfaceLighting` option on `createDungeonRenderer` (defaults: floor=0.85, ceiling=0.95, wallMin=0.9, wallMax=1.1); no torch flicker or tint bands; used by `dungeonRenderer.ts`
