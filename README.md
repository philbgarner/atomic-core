...
- Entity move glide — entities visually glide to their new grid cell over a short tween instead of snapping instantly; tunable via `moveAnimMs` (default `130`, `0` disables) and `moveAnimEasing` on `createDungeonRenderer`; the same glide applies to `ObjectPlacement`s moved via `game.dungeon.moveObject(id, x, z)` (force effects, pushed furniture)
...
- **Fluid simulation** — mass-conserving cellular automaton for water/lava/any fluid pooling on the dungeon floor plan, following real `floorHeightOffset` steps (fast-fills steep drops, then levels out flat across stepped or sloped pits); `fluidFieldFromDungeon()` builds a field from generated dungeon data, `stepFluid()` advances it, `createFluidSurface()` renders it as a live mesh with cross-cell-smoothed surfaces (no stair-stepping between connected cells at different heights) and optional per-fluid-type `refractionIndex`/`opacity`/`glowColor`/`glowIntensity` (live-editable via `setFluidProperty()`) for a wavy-surface look and danger/magic glow telegraphing
